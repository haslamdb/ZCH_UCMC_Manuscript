...
# Custom function for creating discrete SHAP summary plots
def custom_shap_summary_plot(shap_values, features, feature_names=None):
    """
    Create a custom SHAP summary plot with discrete color scales and value labels.
    ...
    feature_order = feature_order[-20:]
    ...
    for i, feature_idx in enumerate(feature_order):
        ...
        if is_categorical:
            # For categorical features, use distinct colors and add labels
            unique_categories = sorted(unique_values)
            ...
            # Create scatter plot points colored by category
            for cat in unique_categories:
                cat_mask = feature_values == cat
                if np.any(cat_mask):
                    plt.scatter(
                        feature_shap_values[cat_mask], 
                        np.ones(np.sum(cat_mask)) * i,
                        color=cmap(category_to_color[cat]),
                        s=20, 
                        alpha=0.6,
                        label=f"{feature_name}={cat}"
                    )
        else:
            # For continuous features, discretize into 5 bins
            bins = np.linspace(np.min(feature_values), np.max(feature_values), 6)
            bin_labels = [f"{bins[i]:.2f}-{bins[i+1]:.2f}" for i in range(len(bins)-1)]
            
            # Digitize feature values
            bin_indices = np.digitize(feature_values, bins[1:-1], right=True)
            ...
            for bin_idx in range(len(bins)-1):
                ...
                if np.any(bin_mask):
                    plt.scatter(
                        feature_shap_values[bin_mask], 
                        np.ones(np.sum(bin_mask)) * i,
                        color=cmap(bin_idx),
                        s=20, 
                        alpha=0.6,
                        label=f"{feature_name}={bin_labels[bin_idx]}"
                    )
    
    # Add feature names as y-axis labels
    plt.yticks(range(len(feature_order)), [feature_names[i] for i in feature_order])
    plt.xlabel("SHAP value (impact on model output)")
    
    # Add a vertical line at x=0
    plt.axvline(x=0, color="gray", linestyle="--", alpha=0.5)
    
    # Create custom legend for the plot with smaller font size
    # We need to handle the legend carefully as there might be many categories
    handles, labels = plt.gca().get_legend_handles_labels()
    by_label = dict(zip(labels, handles))
    if len(by_label) <= 15:  # Only show legend if not too crowded
...
for target_microbe in key_organisms:
    ...
    try:
        ...
        # Use custom SHAP summary plot with discrete colors and labels
        try:
            plt.figure(figsize=(12, 10))
            custom_shap_summary_plot(shap_values, X_test, feature_names=X_encoded.columns.tolist())
            plt.title(f"SHAP Summary with Discrete Color Scale: {target_microbe}")
            plt.tight_layout()
            plt.savefig(f"../results/shap_summary_{target_microbe}.pdf", bbox_inches="tight")
            plt.close()
        except Exception as e:
            print(f"⚠️ Error creating custom SHAP summary plot: {e}")
            # Fallback to original SHAP plot
            try:
                ...
                plt.tight_layout()
                plt.savefig(f"../results/shap_summary_{target_microbe}_fallback.pdf", bbox_inches="tight")
                plt.close()
            except Exception as e2:
                print(f"⚠️ Error creating fallback SHAP plot: {e2}")
