...
if __name__ == "__main__":
    ...
    # Run LMM analysis on the entire microbiome community
    ...
    summary_rows = []
    for pc, results_pc in lmm_results.items():
        # Get top 3 significant predictors
        top_predictors = results_pc['coefficients'][results_pc['coefficients']['Significant']]
        top_3 = top_predictors.head(3)['Variable'].tolist() if len(top_predictors) > 0 else []
        
        summary_rows.append({
            'PC': pc,
            'R-squared': results_pc['r_squared'],
            'AIC': results_pc['aic'],
    ...
    # Use the normalized_df from results
    ...
    # Create a summary table of key results
    glmm_summary_rows = []
    for pc, results_pc in glmm_results.items():
        # Get top 3 significant predictors
        top_predictors = results_pc['coefficients'][results_pc['coefficients']['Significant']]
        top_3 = top_predictors.head(3)['Variable'].tolist() if len(top_predictors) > 0 else []
