...
# Additional sample key for antibiotics
sample_key_abx_file <- file.path(DATA_DIR, "SampleKeyAbx20231106.csv")
NewSampleKeyAbx <- read.csv(sample_key_abx_file, header = FALSE)
...
stool_week_test <- pairwise.wilcox.test(DiversityStool$Shannon, DiversityStool$SampleCollectionWeek)

# Kruskal-Wallis test for stool
kruskal_stool <- kruskal.test(DiversityStool$Shannon, DiversityStool$SampleCollectionWeek)
print("Kruskal-Wallis test for stool diversity by week:")
print(kruskal_stool)

# Diversity by gestational age and location
...
DiversityAgeLocation <- ggplot(subset(Diversity, Diversity$SampleType %in% c("Stool") &
                             ...
                             aes(x=GestationCohort, y=Shannon, fill=GestationCohort)) + 
  geom_boxplot(lwd=1, aes(color=factor(GestationCohort), fill=NA), outlier.size=3) +
  stat_summary(fun=mean, geom="point", shape=5, size=8) + 
  scale_fill_manual(values=col) + 
  scale_colour_tableau() +
  geom_point(size=4, aes(color=factor(GestationCohort))) + 
  ylab("Shannon Diversity Index : Stool\n") + 
  paramsAngled() +
  xlab(NULL) + 
  facet_grid(rows=Location ~ SampleCollectionWeek)

ggsave(DiversityAgeLocation, file=file.path(RESULTS_DIR, "ShannonGestationalAge.pdf"), width=7, height=12, limitsize=FALSE)

# Diversity by location and sample type
DiversityLocationSample <- ggplot(subset(Diversity, !is.na(Diversity$GestationCohort)),
                               aes(x=Location, y=Shannon, fill=Location)) + 
  geom_boxplot(lwd=1, aes(color=factor(Location), fill=NA), outlier.size=3) +
  stat_summary(fun=mean, geom="point", shape=5, size=8) + 
  scale_fill_manual(values=col) + 
...
ggsave(DiversityLocationSample, file=file.path(RESULTS_DIR, "ShannonLocationType.pdf"), width=7, height=12, limitsize=FALSE)

# Diversity by gestational age, sample type, and collection week
...
DiversityAgeSampleType <- ggplot(subset(Diversity, Diversity$SampleType %in% c("Axilla", "Groin", "Stool") &
                                      !is.na(Diversity$GestationCohort)),
                              aes(x=GestationCohort, y=Shannon, fill=TypeWeek)) + 
  geom_boxplot(lwd=1, aes(color=factor(TypeWeek), fill=NA), outlier.size=3) +
  stat_summary(fun=mean, geom="point", shape=5, size=8) + 
  scale_fill_manual(values=col) + 
  scale_colour_tableau() +
  ...
  paramsAngled() +
  xlab(NULL) + 
  facet_grid(. ~ SampleType + SampleCollectionWeek + Location)

ggsave(DiversityAgeSampleType, file=file.path(RESULTS_DIR, "ShannonAgeSampleWeek.pdf"), width=20, height=8, limitsize=FALSE)

# Statistical tests for diversity by gestational age
gestation_axilla_test <- pairwise.wilcox.test(DiversityAxilla$Shannon, DiversityAxilla$GestationCohort)
gestation_groin_test <- pairwise.wilcox.test(DiversityGroin$Shannon, DiversityGroin$GestationCohort)
gestation_stool_test <- pairwise.wilcox.test(DiversityStool$Shannon, DiversityStool$GestationCohort)

print("Diversity by gestational age tests:")
print("Axilla:")
print(gestation_axilla_test)
print("Groin:")
print(gestation_groin_test)
print("Stool:")
print(gestation_stool_test)

# Diversity by antibiotics exposure
DiversityStoolAbx <- ggplot(subset(Diversity, Diversity$SampleType %in% c("Stool")),
                         aes(x=PostNatalAntibioticsNew, y=Shannon, fill=PostNatalAntibioticsNew)) + 
  geom_boxplot(lwd=1, aes(color=factor(PostNatalAntibioticsNew), fill=NA), outlier.size=3) +
  stat_summary(fun=mean, geom="point", shape=5, size=8) + 
  scale_fill_manual(values=col) + 
  scale_colour_tableau() +
  geom_point(size=4, aes(color=factor(PostNatalAntibioticsNew))) + 
  ylab("Shannon Diversity Index : Stool\n") + 
  paramsBoxWide() +
  xlab(NULL) + 
  facet_grid(. ~ SampleCollectionWeek + Location)

ggsave(DiversityStoolAbx, file=file.path(RESULTS_DIR, "ShannonAgeAntibiotics.pdf"), width=20, height=8, limitsize=FALSE)

# Save processed data 
save.image(file=file.path(RESULTS_DIR, "NICU_ProcessedData.RData"))

# Analyze location differences in microbial composition
print("Analyzing location differences in microbial composition...")

# Filter and prepare data for location analysis
LocationWeek1Species <- subset(NICUSpeciesNR, 
                            NICUSpeciesNR$Location %in% c("Cincinnati", "Hangzhou") & 
                            NICUSpeciesNR$SampleType == "Stool" &
...
# Multi-response permutation procedure (MRPP) for location effect
LocationWeek1Species.mrpp <- mrpp(LocationWeek1Species[,3:ncol(LocationWeek1Species)], 
                                LocationWeek1Species$Location, 
                                distance = "bray")
print("MRPP for location effect (Week 1, Stool):")
print(LocationWeek1Species.mrpp)

# Perform pairwise Wilcoxon tests for each species by location
# Aggregate results into a table
print("Calculating species differences by location...")

# Start with one species
species_col <- 3  # First species column
Wilcox <- pairwise.wilcox.test(LocationWeek1Species[,species_col], LocationWeek1Species[,2])
WilcoxTable <- as.data.frame(Wilcox$p.value)
names(WilcoxTable) <- names(LocationWeek1Species)[species_col]
...
# Calculate species abundance and differences between locations
...
SpeciesCounts$Fraction <- sapply(1:length(rownames(SpeciesCounts)), function(i) {
  SpeciesCounts[i,1] / TotalCountsAll
})

# Calculate group means for each species
SpeciesGroupMeans <- aggregate(. ~ LocationWeek1Species$Location, 
                            mean, 
                            data=LocationWeek1Species[3:ncol(LocationWeek1Species)])
row.names(SpeciesGroupMeans) <- SpeciesGroupMeans#!/usr/bin/env Rscript
# NICU Microbiome Data Import and Analysis
# 
# This script performs the following operations:
# 1. Imports and processes sample metadata
...
names(SpeciesGroupTable) <- c("Species", "Abundance", "OverallMean", "Cincinnati.Mean", 
                            "Hangzhou.Mean", "LogFC", "p_Unadjusted", "FDR")
...
# Plot significant species log fold changes
FilteredSigSpeciesGroupTable <- subset(SigSpeciesGroupTable, 
                                    SigSpeciesGroupTable$LogFC >= 8 | 
                                    SigSpeciesGroupTable$LogFC <= -3)
...
ggsave(filename = file.path(RESULTS_DIR, "LocationSpecies_LogFC.pdf"), 
     plot = LocationSpecies_LogFC, width = 10, height = 14, limitsize = FALSE)

# Save results
write.csv(SigSpeciesGroupTable, file = file.path(RESULTS_DIR, "SignificantLocationDifferences.csv"))
write.csv(SpeciesGroupTable, file = file.path(RESULTS_DIR, "AllLocationSpeciesDifferences.csv"))

# PCA analysis for location differences
print("Performing PCA analysis for location...")
...
location_pca <- PCA(cts_l2, scale.unit = FALSE, ncp = 5, graph = FALSE)
location_pca_plot <- fviz_pca_ind(location_pca,
                               geom.ind = "text",
                               pointsize = 1.5,
                               point.alph = 0.1,
                               title = "Unsupervised Principal Coordinate Analysis",
                               subtitle = "Stool Samples Grouped by Location",
                               col.ind = grps,
                               addEllipses = TRUE,
                               ellipse.alpha = 0.2,
...
# Save PCA plot
pdf(file.path(RESULTS_DIR, "LocationPCA.pdf"))
print(location_pca_plot)
dev.off()

# Analyze antibiotics effect
print("Analyzing antibiotics effect...")

# Filter data for antibiotics analysis (Week 1 stool samples)
Week1Stools <- subset(NICUSpeciesNR, 
                    NICUSpeciesNR$SampleType == "Stool" & 
                    NICUSpeciesNR$SampleCollectionWeek == "Week.1")
Week1Stools <- Week1Stools[,c(1,10,18:ncol(Week1Stools))]
Week1Stools <- na.omit(Week1Stools)

# MRPP test for antibiotics effect
Week1Stools.mrpp <- mrpp(Week1Stools[,3:ncol(Week1Stools)], 
                       Week1Stools$PostNatalAntibiotics, 
                       distance = "bray")
print("MRPP for antibiotics effect (Week 1, Stool):")
print(Week1Stools.mrpp)

# Perform pairwise Wilcoxon tests for each species by antibiotics status
species_col <- 3  # First species column
Wilcox <- pairwise.wilcox.test(Week1Stools[,species_col], Week1Stools[,2])
WilcoxTable <- as.data.frame(Wilcox$p.value)
names(WilcoxTable) <- names(Week1Stools)[species_col]
row.names(WilcoxTable) <- "Wilcox"
...
# Calculate species abundance and differences between antibiotics groups
...
# Calculate group means for each species
SpeciesGroupMeans <- aggregate(. ~ Week1Stools$PostNatalAntibiotics, 
                            mean, 
                            data=Week1Stools[3:ncol(Week1Stools)])
row.names(SpeciesGroupMeans) <- SpeciesGroupMeans#!/usr/bin/env Rscript
# NICU Microbiome Data Import and Analysis
# 
# This script performs the following operations:
