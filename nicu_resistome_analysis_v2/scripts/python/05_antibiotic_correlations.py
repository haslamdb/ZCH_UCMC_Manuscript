#!/usr/bin/env python3
"""
Antibiotic Exposure - AMR Correlation Analysis
...
def load_data():
    """Load metadata with AMR and antibiotic data"""
    print("Loading data...")
    ...
    print(f"Samples with AMR data: {len(metadata)}")
    print(f"Subjects with antibiotic data: {metadata[metadata['has_abx_data']]['SubjectID'].nunique()}")
...
def correlate_specific_antibiotics(metadata, abx_names):
    ...
    if len(results_df) > 0:
        ...
        # Save
        results_df.to_csv(CORR_DIR / "specific_antibiotic_correlations.tsv", sep="\t", index=False)
...
def main():
    ...
    print("="*60)
    print("\nOutputs:")
    print(f"  - {CORR_DIR / 'total_abx_amr_correlation_by_site.tsv'}")
    print(f"  - {CORR_DIR / 'specific_antibiotic_correlations.tsv'}")
    print("\nFigures:")
